...
public class PlayerScript : MonoBehaviour
{
	...
	private void PlayerMove()
	{
		...
		if (!(this.jumpRope || this.sweeping || this.hugging))
		{
			...
			{
				this.moveDirection += (this.firstPrize.velocity * 1.2f * Time.deltaTime + (new Vector3(this.firstPrizeTransform.position.x, this.height, this.firstPrizeTransform.position.z) + new Vector3(Mathf.RoundToInt(this.firstPrizeTransform.forward.x), 0f, Mathf.RoundToInt(this.firstPrizeTransform.forward.z)) * 3f - base.transform.position)) * this.principalBugFixer;
			}
	...
	private void StaminaCheck()
	{
		if (this.cc.velocity.magnitude > 0.1f)
		{
			...
			if (this.stamina < 0f && this.stamina > 0f)
			{
				this.stamina = 0f;
			}
		}
		...
		this.SphereBar.fillAmount = this.stamina / this.maxStamina;
	}
	...
	public UnityEngine.AI.NavMeshAgent gottaSweep;

	// Token: 0x04000709 RID: 1801
	public UnityEngine.AI.NavMeshAgent firstPrize;

	// Token: 0x0400070A RID: 1802
	public Transform firstPrizeTransform;

	// Token: 0x0400070B RID: 1803
	public Image SphereBar;
